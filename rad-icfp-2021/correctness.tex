...
\begin{tabular}{l c l}
    ...
    $\seml$!e$_1$!$\icomp$!e$_2$!$\semr$ &=& $\Lambda$($id_\Gamma\concatcomp\seml$!e$_2$!$\semr$; $\Lambda^{-1}$($\seml$!e$_1$!$\semr$)) \\
    ...
    $\seml$!<e$_1$, e$_2$>!$\semr$  &=& !<!$\seml$!e$_1$!$\semr$, $\seml$!e$_2$!$\semr$!>! \\
...
sends a non-Jacobian primitive to a tuple, as in Example \ref{exm:unf}. 
...
This functor computes reverse-mode derivatives. \MH{define more precisely}
Because terms of Source UNF are interpreted in $\ConcatS$, we observe the effect of $\Dsynrevsymbol_{\rho}$ on them
...
\begin{definition}[$\Dsynrevsymbol_{\rho}$ as a lax functor]
    ...
    It naturally extends to sequential composition. 
    It does not automatically extend to a multi-functor \MH{define} between concategories because \\
    ...
    Here, $\widehat{+}$ is the reverse-derivative of the copy map, which is known to be fanout \MH{define}. 
