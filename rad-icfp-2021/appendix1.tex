...
\hline
    ...
    $\directD{\rho}{\Gamma}{Y}$(!map (x.e$_{1}$) e$_{2}$!) &=&  
            ...
            && !let G = (map2 * Z!\\
            && !  (map (a.(!$\grad_\Gamma$!e$_{1}$)[a/x]) A)) in! \\
            && !Y$_{1}$( (x$_{1}$,$\ldots$,x$_n$)$\widehat{+}$(reduce $\widehat{+}$ $\widehat{0}$ G),!\\
...
\hline
    ...
    && !let G = (map2 $\widehat{*}$ Z!\\
    && !  (map (a.(!$\grad_\Gamma$!e$_{1}$)[a/x]) A)) in! \\
    && !Y$_2$( (x$_1$,$\ldots$,x$_n$)!$\widehat{+}$!(reduce $\widehat{+}$ $\widehat{0}$ G),!\\
...
The gradient of !sum(A)! is given by
    ...
    & !let A$_3$ = scanr * 1 A$_1$ in! & \\
    & !map2 * A$_3$ A$_2$! & \\
    = & !let A$_3$ = scanr * 1! & !A$_3$=OnesLike(A)!\\
    &  \quad\quad!OnesLike(shift1L(A)) in! & \\
...
For convenience, let us also rewrite \\
!map2 * B (map (x -> 1) A)! 
...
\begin{tabular}{c l r}
    ...
    & \Big)!(ZerosLike(A),1)!\\
    = & !Y(OnesLike(A))! & (same reduction as previously)\\
    = & !let C$_1$ = map2 (a,b -> b) A B in! & !C$_1$ = B! \\
    & !let C$_2$ = map2 (a,b -> a) A B in! & !C$_2$ = A! \\
    & !(map2 * C$_1$ OnesLike(A),! &\\ 
    & !map2 * C$_2$ OnesLike(A))!  &\\
